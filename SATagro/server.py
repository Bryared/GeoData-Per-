...
# Asegurar que el servidor busque los archivos JSON en las rutas correctas
...
class SatAgroAPIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    # GEMELO DIGITAL ACTIVO: Ecuaciones físicas en caliente en el Servidor
    def handle_live_simulation(self, params):
        # Parámetros recibidos: parcel_id, clima, riego_mm, freatico_cm, corrector
        pid = int(params.get("parcel_id", 1))
        ...
        # Buscar la parcela
        parcela = next((p for p in sim_data["parcelas"] if p["id"] == pid), None)
        if not parcela:
            self.send_error_response(404, f"Parcela {pid} no encontrada.")
            return

        lecturas = sim_data["series_temporales"][str(pid)]
        ultima = lecturas[-1]
        
        # --- SOLVER FÍSICO EN CALIENTE (PINN Logic in server) ---
        hum_fact = 1.0
        evap_fact = 1.0
        if clima == "nino":
            hum_fact = 1.4
        ...
        # Humedad 20cm (reacción rápida al riego)
        ...
        # Salinidad (Ecuación de balance de solutos e intrusión freática)
        capilaridad = 0.0
        ...
        # Calcular residuos físicos de PINN en backend
        dz = 0.20
        dt = 86400.0
        K_sat = 1e-5
        ...
        riesgo = "BAJO"
        if sal_20_nueva > umbral * 1.4:
            riesgo = "CRÍTICO"
        elif sal_20_nueva > umbral * 1.1:
            riesgo = "ALTO"
        elif sal_20_nueva > umbral:
            riesgo = "MODERADO"
            
        if freatico < 85.0 and riesgo in ("BAJO", "MODERADO"):
            riesgo = "ALTO"
    ...
    # DIGITALIZACIÓN EN CALIENTE: Añadir nueva parcela dibujada desde el Dashboard
    def handle_add_parcel(self, params):
        # Campos requeridos: codigo, cultivo, area_ha, lat, lng, coords
        codigo = params.get("codigo")
        cultivo = params.get("cultivo")
        area_ha = float(params.get("area_ha", 4.0))
        lat = float(params.get("lat"))
        lng = float(params.get("lng"))
        coords = params.get("coords") # Polígono [[lat, lng], [lat, lng], ...]
        ...
        # Mapeo de parámetros biofísicos de salinidad según cultivo seleccionado
        umbral_ec = 3.0
        prof_raiz = 40
        drenaje = False
        ...
        # Simular 30 días de lecturas de suelo iniciales para esta nueva parcela
        ...
        # Generar prescripciones iniciales para esta nueva parcela
        ...
        # Guardar archivos actualizados en el disco
